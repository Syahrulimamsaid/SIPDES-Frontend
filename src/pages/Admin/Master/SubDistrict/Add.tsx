...
interface AddSubDistrictModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export default function AddSubDistrictModal({
  isOpen,
  onClose,
  onSuccess,
}: AddSubDistrictModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await subDistrictController.create(form);
      Toast({ message: "Kecamatan baru berhasil ditambahkan!", variant: "success" });
      onSuccess();
      onClose();
    } catch (err) {
